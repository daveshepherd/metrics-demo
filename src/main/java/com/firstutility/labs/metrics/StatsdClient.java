...
public class StatsdClient extends TimerTask {
	...
	private static final Logger log = Logger.getLogger(StatsdClient.class
			.getName());
	...
	public boolean gauge(String key, double magnitude, double sampleRate) {
		final String stat = String.format(Locale.ENGLISH, "%s:%s|g", key,
				magnitude);
		return send(sampleRate, stat);
	}
	...
	private synchronized boolean doSend(String stat) {
		try {
			...
			log.error(String.format("Could not send stat %s to host %s:%d",
					sendBuffer.toString(), _address.getHostName(),
					_address.getPort()), e);
			return false;
		}
	}
	...
	public synchronized boolean flush() {
		try {
			...
			if (sizeOfBuffer == nbSentBytes) {
				...
				log.error(String
						.format("Could not send entirely stat %s to host %s:%d. Only sent %d bytes out of %d bytes",
								sendBuffer.toString(), _address.getHostName(),
								_address.getPort(), nbSentBytes, sizeOfBuffer));
				return false;
			}
